...
public class DirectoryFieldParam extends FieldParam {
	private final TextField text =new TextField();
	private final Button button = new Button("...");
	public DirectoryFieldParam(PropertyParam<File> _prop) {
		...
		button.setOnAction((event)->{
			...
			System.out.println("DirectoryFieldParam.DirectoryFieldParam()");
			DirectoryChooser dirChooser = new DirectoryChooser();
			if(f != null && f.exists()){
				dirChooser.setInitialDirectory(f);
			}
			File ff = dirChooser.showDialog(paramPane.getScene().getWindow());
			if(ff != null){
				_prop.update(ff);
