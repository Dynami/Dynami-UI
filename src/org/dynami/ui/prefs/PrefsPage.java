...
public class PrefsPage {
		...
		for (final Field f : fields) {
			...
			if (parameter != null) {
				...
				try {
					...
					} else if (parameter.type().equals(Prefs.Type.Date)) {
						field = new ParameterDateField(new ParameterProperty<Date>(parameter, f, data));
	...
	private static class ParameterIntegerField extends ParameterField<Integer> {
		...
		public ParameterIntegerField(ParameterProperty<Integer> property) {
			...
			SpinnerValueFactory<Integer> factory = new SpinnerValueFactory.IntegerSpinnerValueFactory(Integer.MIN_VALUE,
					Integer.MAX_VALUE, property.get(), 1);
	...
	private static class ParameterDoubleField extends ParameterField<Double> {
		...
		public ParameterDoubleField(ParameterProperty<Double> property) {
			...
			SpinnerValueFactory<Double> factory = new SpinnerValueFactory.DoubleSpinnerValueFactory(Double.MIN_VALUE,
					Double.MAX_VALUE, property.get().doubleValue(), .1);
			field.setValueFactory(factory);
	...
	private static class ParameterDateField extends ParameterField<Date> {
		final DatePicker field;
	...
	private static class ParameterColorField extends ParameterField<Color> {
		...
		public ParameterColorField(ParameterProperty<Color> property) {
			...
			field.valueProperty().addListener(new ChangeListener<Color>() {
				public void changed(javafx.beans.value.ObservableValue<? extends Color> observable, Color oldValue,
						Color newValue) {
					update();
				};
			});
